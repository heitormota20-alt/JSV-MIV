...
void main(){
  ...
  if(uEnableRipples==1){
    for(int i=0;i<MAX_CLICKS;i++){
      ...
      float waveR=uRippleSpeed*t;float ring=exp(-pow((r-waveR)/uRippleThickness,2.));
...
export function createPixelBlast(container, opts = {}) {
  const {
    variant = 'circle', pixelSize = 6, color = '#004AAD',
    patternScale = 3, patternDensity = 1.2, pixelSizeJitter = 0.5,
    enableRipples = true, rippleSpeed = 0.4, rippleThickness = 0.12,
    rippleIntensityScale = 1.5, speed = 0.6, edgeFade = 0.25,
  } = opts;
